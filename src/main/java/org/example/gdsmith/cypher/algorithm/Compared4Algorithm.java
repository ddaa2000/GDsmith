...
@Deprecated
public class Compared4Algorithm<S extends CypherSchema<G,?>, G extends CypherGlobalState<O, S>,
    ...
    public Compared4Algorithm(CypherProviderAdapter<G, S, O> provider) {
        super(provider);
    }
...
//            System.exit(0);
    ...
    public void generateDatabase(G globalState) throws Exception{
        TreeBasedGraphGeneratorCompared<G,S> generator = new TreeBasedGraphGeneratorCompared<G,S>(globalState);
        this.subgraphManager = generator.getSubgraphManager();
        List<CypherQueryAdapter> queries = generator.createGraph(globalState);

        TreeBasedGraphGeneratorCompared<G,S> graphGenerator = new TreeBasedGraphGeneratorCompared<>(globalState);
